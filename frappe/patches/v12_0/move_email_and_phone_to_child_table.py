...
def execute():
    ...
    if frappe.is_oracledb:
        contact_details = frappe.db.sql(
			f"""
                ...
                FROM {frappe.conf.db_name}."tabContact" "tabContact"
                WHERE NOT EXISTS (
                    SELECT * FROM {frappe.conf.db_name}."tabContact Email" "tabContact_Email"
                    WHERE "tabContact_Email"."parent" = "tabContact"."name"
                    AND "tabContact_Email"."email_id" = "tabContact"."email_id"
                )
                """,
			as_dict=True,
		)
    ...
    for count, contact_detail in enumerate(contact_details):
        ...
        if frappe.is_oracledb:
            if email_values and (
                count % 10000 == 0 or count == len(contact_details) - 1
            ):
                frappe.db.sql(
                    f"""
						INSERT INTO {frappe.conf.db_name}."tabContact Email"
							("idx", "name", "email_id", "parentfield", "parenttype", "parent", "is_primary", "creation",
							"modified", "modified_by")
						VALUES {", ".join(["%s"] * len(email_values))}
						""",
                    tuple(email_values),
                )
