				...
				try:
					if isinstance(condition, dict):
						result = frappe.get_list(
							d, fields=["count(1) count" if frappe.is_oracledb else "count(*) as count"], filters=condition, ignore_ifnull=True
						)[0].count
					else:
...
def get_notification_info():
	...
	if frappe.is_oracledb:
		doctype_info = dict(frappe.db.sql(f"""select "name", "module" from {frappe.conf.db_name}."tabDocType" """))
	else:
